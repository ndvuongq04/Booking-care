...
public class ServiceMedicalRecordsController {
    ...
    public ResponseEntity<ResServiceMedicalRecordDTO> getById(@PathVariable Long id) throws IdInvalidException {
        ...
        if (smr == null) {
            throw new IdInvalidException("ServiceMedicalRecord với id : " + id + " không tồn tại");
        }
        return ResponseEntity.ok(serviceMedicalRecordsService.toDto(smr));
    }
    ...
    public ResponseEntity<List<ResServiceMedicalRecordDTO>> getServicesByMedicalRecordId(
        ...
        MedicalRecord mr = this.medicalRecordsService.fetchMedicalRecordById(medicalRecordId);

        if (mr == null) {
            throw new IdInvalidException("MedicalRecord với id : " + medicalRecordId + " không tồn tại");
        }
    ...
    public ResponseEntity<List<ResServiceMedicalRecordDTO>> addMultipleServices(
        ...
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(list.stream().map(item -> this.serviceMedicalRecordsService.toDto(item)).toList());
    }
